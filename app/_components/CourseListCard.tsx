                ...
                <CardContent className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-2 mt-2">
                        <div className="flex justify-between text-sm font-sans font-medium text-slate-600">
                            <span>Progress ({courseItem.progressPercentage || 0}%)</span>
                            <span>{courseItem.completedChapters || 0} / {courseItem.courseLayout?.totalChapters || courseItem.courseLayout?.chapters?.length || 0} Ch</span>
                        </div>
                        <div className="w-full h-3 bg-slate-100 wobbly-border overflow-hidden relative">
                            <div 
                                className="h-full bg-sketch-primary transition-all duration-300"
                                style={{ width: `${courseItem.progressPercentage || 0}%` }}
                            />
                        </div>
                        <div className="flex justify-between text-xs font-sans text-slate-400 mt-0.5">
                            <span>{courseItem.remainingChapters || 0} chapters left</span>
                        </div>
                    </div>
                    ...
                    <div className='flex justify-between items-center border-t border-dashed border-slate-200 pt-4'>
                        ...
                        <Link href={`/course/${courseItem?.courseId}`}>
                            <button className="bg-black text-white px-5 py-2 wobbly-border hard-shadow-sm hover:translate-x-0.5 font-display text-lg hover:translate-y-0.5 transition-all flex items-center gap-2 cursor-pointer">
