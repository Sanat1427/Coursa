...
export async function POST(req: NextRequest) {
    try {
        ...
        try {
            ...
            console.warn("[generate-video-content] Inngest send failed. Falling back to direct background execution...", inngestErr?.message || inngestErr);
            
            // Asynchronously execute the direct generation in the background
            Promise.resolve().then(async () => {
                try {
                    await generateChapterContentDirect({
                        chapter,
                        courseId,
                        courseName: courseRow[0].courseName,
                        language: courseRow[0].language
                    ...
                    console.error("[generate-video-content] Direct background fallback execution failed:", fallbackErr);
                }
            });
        }
