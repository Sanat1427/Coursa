...
import { 
    Loader2, BrainCircuit, RefreshCw, Award, AlertTriangle, 
    ShieldCheck, Sparkles, Play, History, ChevronLeft, 
    BookOpen, Layers
} from "lucide-react";
...
// Dynamic import for KnowledgeGraphView to lazy load the heavy SVG canvas in the client
const KnowledgeGraphView = dynamic(() => import("@/components/KnowledgeGraphView"), {
    ssr: false,
    loading: () => (
        <div className="h-[600px] flex flex-col items-center justify-center bg-[#fbf9f5] wobbly-border border-2 font-display text-slate-400 italic">
            <Loader2 className="w-6 h-6 animate-spin text-sketch-primary mb-2" />
            Loading interactive knowledge map...
        </div>
    )
});
...
export default function RevisionDashboardClient({ initialInsights, initialGraphData }: Props) {
    const router = useRouter();

    const [insights, setInsights] = useState<any>(initialInsights);
    const [graphData, setGraphData] = useState<{ nodes: any[]; edges: any[] }>(initialGraphData);
    ...
    const fetchData = async () => {
        ...
        try {
            const [insightsRes, graphRes] = await Promise.all([
                axios.get("/api/learning-insights?refresh=true"),
                axios.get("/api/knowledge-graph?refresh=true")
            ]);
